...
export function GlobalReach() {
  const { svg, pin, label } = generateWorldMap();
    ...
    <section id="locations" className="border-y border-border/70 bg-muted/40">
      <div className="mx-auto max-w-6xl px-6 py-24 md:py-32">
        ...
        <ScrollReveal delay={0.1}>
          <HoverCard className="rounded-2xl border border-border bg-card p-6 md:p-10">
            <div
            ...
            >
              ...
              />
              {pin && <MapPin xPct={pin.xPct} yPct={pin.yPct} label={label} />}
            </div>
          </HoverCard>
        </ScrollReveal>
      </div>
